...
export default function Login({ onLoginSuccess, onTogglePage, showToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast('Network error connecting to auth server.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#fbfaf7] text-stone-900 flex flex-col items-center justify-center p-4">
      {/* Background Blobs (subtle warm light overlay) */}
      ...
      <div className="w-full max-w-md bg-white border border-stone-200 rounded-2xl p-8 shadow-sm backdrop-blur-md relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-xl font-bold tracking-tight text-stone-900">Welcome to LinkFlow</h2>
          <p className="text-xs text-stone-500 mt-1">Sign in to manage your automated LinkedIn pipeline</p>
        </div>
